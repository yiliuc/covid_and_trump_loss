...
election_2016_republican <- read.csv(here("outputs/data/election_data_2016.csv")) %>% 
  filter(party == "Republican") %>% 
  dplyr::select(fips, pct_vote_2016)

republican_data <- data %>% 
  filter(party == "Republican") %>% 
  inner_join(election_2016_republican, by = "fips") %>% 
  mutate(change_vote = pct_vote - pct_vote_2016)
```
...
library(Matching)
# Propensity score matching
rr <- Match(Y = republican_data$change_vote, Tr = republican_data$treatment, 
...
# PSS
sub <- quantile(republican_data$prop_score, probs = c(0.2, 0.4, 0.6, 0.8))

sub1 <- glm(pct_vote ~ high_infrate, 
            data = republican_data[republican_data$prop_score <= sub[1], ])
sub2 <- glm(pct_vote ~ high_infrate, 
            data = republican_data[republican_data$prop_score > sub[1] & republican_data$prop_score <= sub[2], ])

sub3 <- glm(pct_vote ~ high_infrate, 
            data = republican_data[republican_data$prop_score > sub[2] & republican_data$prop_score <= sub[3], ])

sub4 <- glm(pct_vote ~ high_infrate, 
            data = republican_data[republican_data$prop_score > sub[3] & republican_data$prop_score <= sub[4], ])

sub5 <- glm(pct_vote ~ high_infrate, 
            data = republican_data[republican_data$prop_score > sub[4], ])
...
extract_info <- function(model, quantile) {
  ...
  data.frame(
    quantile = quantile,
    ce = coef_summary["high_infrate", "Estimate"],
    p_val = coef_summary["high_infrate", "Pr(>|t|)"]
  )
}
...
#| message: false
# Propensity score regression adjustment
model_adj <- glm(pct_vote ~ treatment + prop_score, data = republican_data)
# summary(model_adj)
```
